		...
		success : function(res){
			...
			for(var i = 0; i < res.length; i++){
				...
				if(res[i].msg_count > 0){
					room += `	<div class="counter" style="display: inline; color: red; font-weight:bold; font-size: 18px;">\${res[i].msg_count }</div>`;
				}
				room += `	</div>`;
			}
			$(".chatList").html(room);
...
// * 3 메시지 태그 append
 function appendMessageTag(LR_className, id, msg, name, rdate, unread_count){
	...
	const chatList = createMessageTag(LR_className, id, msg, name, rdate, unread_count);
 ...
 function createMessageTag(LR_className, id, msg, name, rdate, unread_count){
	...
	if(unread_count > 0){
		if((LR_className == "right")){
			chatList +=			`<span class="cnt">\${unread_count}</span> <span class="msg">\${msg}</span>`
		}else{
			chatList +=			`<span class="msg">\${msg}</span> <span class="cnt">\${unread_count}</span>`
		}
	}else{
